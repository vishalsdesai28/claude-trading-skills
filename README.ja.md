...
- **Intrinsic Value (DCF)** (`intrinsic-value-dcf`): FMPファンダメンタル + 10年米国債リスクフリーレートを使用。キーレスなyfinanceフォールバック（`--yfinance`）または`--input-json`によるオフライン実行に対応
- **Dynamic Exit Engine** (`dynamic-exit-engine`): 🟡 オプション — オフラインのトレーリングストップFSM。FMP日次OHLCVは任意で、`--bars-source fixture`（Alpaca/Robinhood/FMPエクスポート）はキーレス
- **Dealer Gamma Analyzer** (`dealer-gamma-analyzer`): APIキー不要（無料のCBOE約15分遅延オプションJSON）
- **Short-Squeeze Radar** (`short-squeeze-radar`): APIキー不要（無料のFINRA Reg SHO日次ファイル + 任意のショートインタレストエクスポート）
- **Retail Sentiment Ingestor** (`retail-sentiment-ingestor`): APIキー不要（キーレスのStockTwits + Reddit）。任意のXパスは`X_API_KEY`経由
