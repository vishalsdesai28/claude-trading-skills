...
| **[Short-Squeeze Radar]({{ '/ja/skills/short-squeeze-radar/' | relative_url }})** | 無料のFINRAデータ（Reg SHO日次ショートボリュームの上昇inflection＋任意の隔週ショートインタレストによるdays-to-cover）で米国株のショートスクイーズ余地をランク付けし、crowded-short / neutral / low-pressure に分類 | <span class="badge badge-free">API不要</span> |
| **[Trend Reclaim Screener]({{ '/ja/skills/trend-reclaim-screener/' | relative_url }})** | 引け後に SMA50 リクレイム（50日線割れ→リセット→再奪回）のロング候補をスクリーニング。リクレイム品質／モメンタム／構造／出来高／トレンド整合の5要素加重スコアでA-Dグレード評価し、翌日寄付エントリー・ストップ・T1のプランレベル付きでトップ3を選出。キーレスのYahoo Financeデータ使用 | <span class="badge badge-free">API不要</span> |
| **[Swing Setup Screener]({{ '/ja/skills/swing-setup-screener/' | relative_url }})** | 7画面一体型の引け後スクリーナー：スイングトレンドのロング／ショート（SMA50/200整列＋トリガー分類）、52週高値圏リーダー（SMA20プルバックプラン）、高ATR%銘柄、翌営業日ウォッチリスト3種（in-play／出来高異常クアドラント／売り圧力を伴う下落）。要素加重A-Dグレードと「直近確定セッションのみ評価」のタイミング契約付き。キーレスのYahoo Financeデータ使用 | <span class="badge badge-free">API不要</span> |
...
| **[Morning Note Briefing]({{ '/ja/skills/morning-note-briefing/' | relative_url }})** | 既存スキルのJSON出力（決算、値動き/セクター、マクロカレンダー、ライブ触媒）を合成し、固定フォーマットの2分プレマーケット・モーニングノートを組み立て。最重要トピック、方向性のあるTop Call、ロング/ショートのアイデアを提示。純粋なオフラインアセンブラ | <span class="badge badge-free">API不要</span> |
...
| **Index Futures Weekly Income** | ES/NQ先物・オプションの週次インカムシグナルを生成。レジーム判定（トレンド + VIX/VXN）でロング限定のプルバック/ブレイクアウト（1.5R）とプットクレジットスプレッドを提示。2010-2026年のYahoo Financeデータでバックテスト済み、下落トレンド・高ボラ時はスタンドアサイド | <span class="badge badge-free">API不要</span> |
...
| **[Dealer Gamma Analyzer]({{ '/ja/skills/dealer-gamma-analyzer/' | relative_url }})** | 無料のCBOE約15分遅延オプションデータからディーラーのオプションガンマ（GEX）を定量化。ストライク別の符号付きガンマ、コールウォール/プットウォールをS/R価格として提示し、ガンマフリップ、max pain、ポジティブ/ネガティブガンマ・レジーム分類を出力 | <span class="badge badge-free">API不要</span> |
...
| **[Momentum DCA Bot]({{ '/ja/skills/momentum-dca-bot/' | relative_url }})** | 完全メカニカルな日次モメンタムDCAスリーブ。キーレスYahooのモメンタム押し目スクリーン、SPY > 50DMAのレジームゲート下でAlpaca（デフォルトはペーパー）経由の$300/日定額買付、+20%で半分利確、50DMA割れまたは15%トレーリングストップで全売却、追記専用トレードログ | <span class="badge badge-api">Alpaca必須</span> <span class="badge badge-optional">FINVIZ任意</span> |
...
| **[Intrinsic Value (DCF)]({{ '/ja/skills/intrinsic-value-dcf/' | relative_url }})** | 米国株の本源的価値を3手法でトライアンギュレート。CAPM-WACCのDCF（5年FCFFフェード、デュアルターミナルバリュー、再計算感応度グリッド）、ピアメディアン相対倍率、任意のsum-of-the-parts、Bull/Base/Bearシナリオ、セクター別ルーティング。キーレスなyfinanceフォールバック対応 | <span class="badge badge-api">FMP必須</span> |
| **[Dynamic Exit Engine]({{ '/ja/skills/dynamic-exit-engine/' | relative_url }})** | 保有ロングポジションの適応型トレーリングエグジットを管理。2フェーズ（ハードストップ→ラチェット式プロフィットフロア）に建値ロック、stale-flatタイムアウト、ATRノイズバンド、連続ブレイク確認を備え、OHLCV系列を決定論的にリプレイ | <span class="badge badge-optional">FMP任意</span> |
| **[Adversarial Trade Debate]({{ '/ja/skills/adversarial-trade-debate/' | relative_url }})** | スクリーナー候補を2段階の敵対的ディベートを通じて決定的な確信度レーティングとサイズ付き・リスク検証済みのトレード判断に変換。強気vs弱気ディベートを5段階レーティングに判定し、続いてPMが判定するaggressive/conservative/neutralのリスクディベートで最終判断を確定。バリュエーション/テクニカル/センチメント出力を消費し、position-sizerへ受け渡し | <span class="badge badge-free">API不要</span> |
...
| **[Social Signal Ingestor]({{ '/ja/skills/social-signal-ingestor/' | relative_url }})** | YouTubeチャンネルの字幕（yt-dlp）から取引シグナルを取り込み、ティッカーごとに1つのシグナルノートをローカルvaultへ抽出し、エッジパイプライン向けの機械可読シグナルインデックスを生成。v1はYouTube、X/RedditはAgent-Reach経由で後日対応 | <span class="badge badge-free">API不要</span> |
| **[Edge Social Aggregator]({{ '/ja/skills/edge-social-aggregator/' | relative_url }})** | social-signal-ingestorのソーシャルシグナルを直近性×裏付け数でスコアリングし、ティッカーごとに集約してedge-signal-aggregator向けフィードを生成。動画間で同一ティッカーを重複排除。クロスソース統合やランキングは行わない | <span class="badge badge-free">API不要</span> |
| **[Ticker Enricher]({{ '/ja/skills/ticker-enricher/' | relative_url }})** | ティッカーシグナルに企業名/セクター/業種＋推奨時・現在株価（Yahoo Finance）、騰落率・保有日数を付与し、汎用ライター向けのレコードファイルを出力。ティッカーを持つ任意のプロデューサーで再利用可能 | <span class="badge badge-free">API不要</span> |
...
| **[Retail Sentiment Ingestor]({{ '/ja/skills/retail-sentiment-ingestor/' | relative_url }})** | キーレスのStockTwits・Reddit（任意でX）からティッカー別のリテールセンチメントをリアルタイム取得し、決定論的にスコアリングしたシグナルノートを共有ソーシャルvaultへ出力。クロスソース乖離と>=90/10の過熱フラグ付き | <span class="badge badge-free">API不要</span> |
...
| **[Write Supabase]({{ '/ja/skills/write-supabase/' | relative_url }})** | 汎用Supabaseテーブルライター。レコードJSONファイルを読み込み、呼び出し側指定のテーブル・コンフリクトキーでupsert/insert。ドメイン非依存で、Supabaseへ永続化する任意のワークフローで再利用 | <span class="badge badge-free">API不要</span> |
| **[Live Analytics Dashboard]({{ '/ja/skills/live-analytics-dashboard/' | relative_url }})** | 他スキルのreports/*.json（ウォッチリスト、市場幅、ポートフォリオ監視）をダッシュボードとして描画。レポート形状を検出しKPI + ソート可能テーブルを構築、スナップショットArtifactとして公開またはライブ更新のローカルページを配信。CSP安全性ゲート付き、有料データ不要 | <span class="badge badge-free">API不要</span> |
...
| Intrinsic Value (DCF) | 必須 | - | - |
| Dynamic Exit Engine | 任意 | - | - |
| Data Quality Checker | 任意 | - | - |
| Position Sizer | 任意 | - | - |
| Momentum Dca Bot | - | 任意 | 必須 |
| その他すべてのスキル | - | - | - |
