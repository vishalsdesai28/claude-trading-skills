...
| **[Social Signal Ingestor]({{ '/en/skills/social-signal-ingestor/' | relative_url }})** | Ingests trading signals from YouTube channels via yt-dlp transcripts; extracts one signal note per ticker into a local vault and emits a machine-readable signal index for the edge pipeline. YouTube is v1; X/Reddit plug in later via Agent-Reach | <span class="badge badge-free">No API</span> |
