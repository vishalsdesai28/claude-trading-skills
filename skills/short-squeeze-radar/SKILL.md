...
```json
{
  ...
  "lookback_days": 5,
  "short_interest_source": "short_interest.csv",
  "squeeze_primed": ["GME"],
  ...
  "candidates": [
    {
      ...
      "latest_ratio": 0.68,
      "ratio_series": [0.48, 0.56, 0.68],
      "ratio_trend": "rising",
      "rising_inflection": true,
      "days_to_cover": 8.33,
      "short_interest": 25000000,
      "avg_daily_volume": 3000000,
      "classification": "crowded_short",
      "squeeze_score": 89.6,
      "squeeze_primed": true,
      "fallback_used": false,
      "notes": ["crowded short — squeeze fuel for a long", "days-to-cover 8.33"]
    }
  ]
}
```
...
A ranked candidate table (rank, symbol, class, score, days-to-cover, short-volume ratio, trend, primed) plus per-symbol detail with the multi-day ratio series and notes. Reports save to `reports/short_squeeze_YYYY-MM-DD_HHMMSS.json` and `.md`.

## Scoring and Classification

- **short_volume_ratio** = (ShortVolume + ShortExemptVolume) / TotalVolume, latest available day, with a **prior-trading-day fallback** when a symbol is missing from the most recent file.
- **classification**: `crowded_short` (ratio ≥ 0.60 or days-to-cover ≥ 5.0), `low_pressure` (ratio ≤ 0.35 and days-to-cover < 2.0), else `neutral`.
- **squeeze_score** (0–100): short-volume ratio (up to 40) + days-to-cover (up to 40) + trend (rising 20 / flat 8 / falling 2). No short interest → days-to-cover component is 0, score caps at 60.
- **squeeze_primed**: crowded short AND rising short-volume ratio AND (days-to-cover ≥ 3.0 or ratio ≥ 0.65).
- **ranking**: by days-to-cover (descending; unknown last), then squeeze score, then latest ratio.

## Resources

- `references/squeeze_signals.md`: Short-squeeze mechanics, FINRA data sources and file formats, how to read short-volume ratio vs. short interest, thresholds, and caveats.
- `scripts/scan_short_squeeze.py`: Fetch + parse + rank CLI. Pure parsing/analytics functions are unit-tested against saved FINRA fixtures (no network).

## Key Principles

1. **The radar finds fuel, not ignition.** A crowded short is a *necessary* condition for a squeeze, not a sufficient one — a squeeze needs a demand catalyst (news, gamma, buy pressure).
2. **Short volume ≠ short interest.** Daily short volume is executed off-exchange short *flow* (a proxy); short interest is reported *shares held short*. Use both.
3. **High is the tail.** Market-wide short volume routinely runs ~40–50%; the squeeze-relevant signal is the upper tail (≥ 60%) and a rising trend.
4. **Days-to-cover is the pressure gauge.** More days-to-cover = more forced buying required to unwind = more squeeze potential.
5. **Confirm borrow and catalyst.** Verify hard-to-borrow status and a real catalyst before acting; free FINRA data lags intraday reality.
