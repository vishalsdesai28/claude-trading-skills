...
- `reports/debate_brief_<TICKER>_<ts>.{json,md}` — assembled analyst inputs, with
  present/missing lanes tracked (Step 1).
- The two `ResearchPlan` / `PortfolioDecision` judgments — as structured JSON or
  the deterministic-header markdown in `references/schemas.md`.
- `reports/debate_decision_<ts>.json` — the recovered structured decision plus
  the `position_sizer_handoff` block (Step 8).

## Consuming and Handing Off (skill wiring)

| Direction | Skill | What is exchanged |
|---|---|---|
| Consumes | `intrinsic-value-dcf` | valuation JSON → the valuation lane |
| Consumes | `technical-analyst` | chart report → the technicals lane |
| Consumes | `retail-sentiment-ingestor` | run report `results[]` → the sentiment lane |
| Consumes (optional) | `market-news-analyst` | news text → the news lane |
| Consumes (optional) | `trader-memory-core` | postmortem lessons → the PM's "prior lessons" block |
| Hands to | `position-sizer` | final entry + stop + risk% → share count |

The wiring is one-directional and by **documented output shape** only — no
sibling module is imported.

## Resources

- `references/debate_protocol.md` — the turn-count gates, rotation order,
  rebut-first rules, and the anti-fence-sitting rule for both stages.
- `references/schemas.md` — `ResearchPlan`, `TraderProposal`, `PortfolioDecision`
  structured schemas plus the deterministic-header free-text fallback.
- `scripts/debate_kit.py` — `assemble` (fold analyst outputs into a brief) and
  `parse-decision` (recover the structured decision from headers and build the
  position-sizer hand-off). Pure stdlib, offline, tested.

## Key Principles

1. **Adversarial by construction.** Every turn rebuts the opponent's last point;
   a turn that only re-lists its own thesis is wasted.
2. **Commit, don't fence-sit.** `Hold` is earned by a genuinely balanced debate,
   not defaulted to under uncertainty.
3. **Reason only from present lanes.** The brief marks missing lanes; never
   fabricate a valuation, level, or sentiment the inputs don't contain.
4. **The judge may re-size.** The Portfolio Manager can move the rating or
   share count off the proposal, but only with a specific risk-debate reason.
5. **Deterministic edges, LLM middle.** Input assembly and decision parsing are
   reproducible; the debate itself is the LLM's job.
6. **Not financial advice.** A decisive rating is a research artifact to
   cross-check, not an instruction to trade.
